...
export default function TrackingScreen({ navigation }) {
  ...
  const renderItem = ({ item }) => (
    <Card
      ...
      onPress={() =>
        navigation.navigate('ShipmentDetail', { shipmentId: item.id || item._id })
      }
    >
        ...
        <View style={styles.deliveryInfo}>
          ...
          <Text style={[styles.deliveryTime, { color: theme.textSecondary }]}>
            {item.estimated_delivery
              ? `Entrega: ${truncate(item.estimated_delivery, 16)}`
              : 'Sin estimación'}
          </Text>
        </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.headerTitle, { color: theme.text }]}>Rastrear envío</Text>
      ...
      <View style={styles.searchContainer}>
        <View style={[styles.searchBar, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          ...
          <TextInput
            ...
            placeholder="Buscar por folio, tracking o dirección"
      ...
      {loading ? (
        ...
        <EmptyState
          icon={<Ionicons name="cloud-offline-outline" size={64} color={colors.error[400]} />}
        ...
        <EmptyState
          ...
          title={searchQuery ? 'Sin resultados' : 'Sin envíos'}
          subtitle={
            searchQuery
              ? 'No se encontraron envíos con ese criterio'
              : 'Crea tu primer envío para comenzar a rastrear'
          }
        />
      ) : (
        <FlatList
